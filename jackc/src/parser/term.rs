...
pub fn parse_term_or_die(stream: &mut Stream) -> Result<(Term, Location)> {
    parse_term(stream).unwrap_or_else(|| stream.unexpected_token_result("expected term"))
}

pub fn parse_term(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
...
fn parse_term_integer_const(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
    trace!(stream, "parse_term_integer_const", {
        stream.consume_if_integer().map(|t| {
            t.integer()
                .map(|n| (Term::Integer(n), t.location()))
                .ok_or_else(|| stream.unexpected_token_err("expected integer"))
        })
    });
}

fn parse_term_string_const(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
    trace!(stream, "parse_term_string_const", {
        stream.consume_if_str().map(|t| {
            t.str()
                .map(|n| (Term::Str(n), t.location()))
                .ok_or_else(|| stream.unexpected_token_err("expected string"))
        })
    });
}

fn parse_term_keyword_const(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
            ...
            .map(|t| {
                match t.keyword() {
                    ...
                    _ => stream.unexpected_token_result(
                        "expected keyword('true', 'false', 'null', 'this')",
                    ),
                }
                .map(|term| (term, t.location()))
            })
    });
}

fn parse_term_var(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
...
fn parse_term_expr(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
    trace!(stream, "parse_term_expr", {
        stream.consume_if_symbol('(').map(|t| {
            expr::parse_expr_or_die(stream).and_then(|expr| {
                stream
                    .ensure_symbol(')')
                    .and(Ok((Term::Expr(expr), t.location())))
            })
        })
    });
}

fn parse_term_unary(stream: &mut Stream) -> Option<Result<(Term, Location)>> {
        ...
        stream.consume_if_symbols(&['-', '~']).map(|t| {
            t.symbol()
                .map(|sym| {
                    ...
                    let (term, _loc) = parse_term_or_die(stream)?;
                    Ok((Term::Unary(unary, Box::new(term)), t.location()))
                })
